...
print("• Cargando tensores de Vision Tower en FP32 desde los safetensors locales...")
weights = {}
for s in shards:
...
print(f"✓ {len(weights)} tensores de visión cargados.")

img = Image.open(img_path).convert("RGB")
W_orig, H_orig = img.size
print(f"✓ Imagen original test.jpg: {W_orig}x{H_orig} píxeles.")
...
pixel_values = torch.tensor(np.array(patches), dtype=torch.float32)
...
print(f"✓ prompt_input.bin compilado: {len(secuencia_final)} tokens ({n_merged} parches visuales).")
