...
# Restaurar el despacho estándar limpio del LM Head
old_fused_head = """            // 🚀 FUSED LM-HEAD: RMSNorm + GEMV en 1 solo paso de silicio sin tocar DRAM
...
restored_clean_head = """            [encHead setComputePipelineState:psoNorm];
            [encHead setBuffer:bufZ offset:0 atIndex:0];
            [encHead setBuffer:bufFinalNorm offset:0 atIndex:1];
...
code = code.replace(old_fused_head, restored_clean_head)

with open("src/main_aether.mm", "w") as f:
    f.write(code)

print("✓ LM Head restaurado al modo limpio y numéricamente exacto.")
