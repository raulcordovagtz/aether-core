...
with open("spec/experiments/C007_multiscale_01.yaml", "r") as f:
...
D = p["D"]
R = p["R"]
PREFILL_STEPS = p["prefill_spin_steps"]
DECODE_STEPS = p["decode_spin_steps"]
...
print(f"=================================================================================")
print(f" ⚙️ TRANSPILADOR AOT MULTIESCALA :: {exp_id}")
print(f"=================================================================================")
print(f"• Pasos Prefill: {PREFILL_STEPS} | Pasos Decode: {DECODE_STEPS}")
print(f"• κ = {KAPPA:.12f} | Δτ = {DT:.8f}")

# 1. Emitir Metal Shader Canónico
metal_code = f"""// ═════════════════════════════════════════════════════════════════════════════
// 🌌 AETHER-VL :: CANONICAL MULTISCALE KERNEL [{exp_id}]
// ═════════════════════════════════════════════════════════════════════════════
...
print("✓ metal/aether_c007_spinor_integrator.metal compilado.")

# 2. Actualizar la invocación en C++ en src/main_aether.mm
with open("src/main_aether.mm", "r") as f:
    code = f.read()

# Actualizar el bucle de pasos en decode
old_decode_loop = "for (uint32_t s = 0; s < 64; ++s)"
new_decode_loop = f"for (uint32_t s = 0; s < {DECODE_STEPS}; ++s)"

if old_decode_loop in code:
    code = code.replace(old_decode_loop, new_decode_loop)
    print(f"✓ Decode ajustado a {DECODE_STEPS} paso(s) por token.")
elif f"for (uint32_t s = 0; s < {DECODE_STEPS}; ++s)" in code:
    print(f"• Decode ya estaba configurado en {DECODE_STEPS} paso(s).")
else:
    print("⚠ No se encontró la firma del bucle decode.")

with open("src/main_aether.mm", "w") as f:
    f.write(code)

print("✓ src/main_aether.mm sincronizado con C007-MULTISCALE-01.")
